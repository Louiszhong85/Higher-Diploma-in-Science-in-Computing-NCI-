...
<c:if test="${empty v.pic}">No Image</c:if></td>
       <td >${v.stockArea}</td>
<td>
<a  style="text-decoration:none" href="<%=path %>/productEdit?id=${v.id}"><i class="layui-icon">&#xe642;</i></a>
  <a  style="text-decoration:none" href="<%=path %>/productDelete?id=${v.id}" onClick="return confirm('Are you want Delete?')"><i class="layui-icon">&#xe640;</i></a>
</td>
		  </tr>
	 </c:forEach>
     </tbody>
